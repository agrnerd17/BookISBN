...
std::istream & operator>>( std::istream & stream, Book & book )
{
  ///////////////////////// TO-DO (21) //////////////////////////////
    ...
    if( stream >> std::ws >> std::quoted( workingItem._isbn ) && stream >> std::ws >> delimeter && delimeter == ',' &&
        stream >> std::ws >> std::quoted( workingItem._title ) && stream >> std::ws >> delimeter && delimeter == ',' &&
        stream >> std::ws >> std::quoted( workingItem._author) && stream >> std::ws >> delimeter && delimeter == ',' && 
...
std::ostream & operator<<( std::ostream & stream, const Book & book )
{
  ...
  stream << std::quoted( book.isbn()   ) << delimeter
         << std::quoted( book.title()  ) << delimeter
         << std::quoted( book.author() ) << delimeter
         << book.price();
  return stream;
  /////////////////////// END-TO-DO (22) ////////////////////////////
}
